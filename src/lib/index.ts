...
const systemMessage = `
  You are a helpful assistant expert in Nutrition.
        ...
        ALWAYS answer like a person in a gentle and friendly manner, giving practical and actionable advice, avoiding technical jargon and complex explanations.
`;
...
async function getSession(client: CopilotClient, sessionId: string) {
  console.debug("Creating or resuming session with ID:", sessionId);

  let session = await client.resumeSession(sessionId, CONFIG).catch(() => {
...
export async function handleSession(
  ...
  let session = await getSession(client, sessionId);

  if (!session) {
    console.debug("Creating new session with ID:", sessionId);
    session = await client.createSession({ sessionId, ...CONFIG });
  } else {
    console.debug("Resumed existing session with ID:", sessionId);
  }
...
export async function resetSession(
  ...
  if (session) {
    ...
    console.debug("Session reset successfully for ID:", sessionId);
  }
}
